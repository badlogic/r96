...
int main(void) {
	...
	struct mfb_window *window = mfb_open("07_rect_opt", image.width * 2, image.height * 2);
	...
	do {
		...
		for (int i = 0; i < 200000; i++) {
			...
			rect_opt(&image, rand() % image.width, rand() % image.width, rand() % (image.width / 5), rand() % (image.height / 5), color);
		}
